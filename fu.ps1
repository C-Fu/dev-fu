...
function Get-StatusCheck {
    ...
    $tools = @(
        ...
        @{Name = "Git"; Cmd = "git"; Args = "--version"; VersionCmd = $true},
        ...
        @{Name = "Winget"; Cmd = "winget"; Args = "--version"; VersionCmd = $true}
    )
...
function Install-DevTools {
    ...
    Write-Host "${DIM}   Node.js, Python, Go, Rust, Bun${NC}"
    Write-Host ""

    Write-Host "${YELLOW}  → This will install: Node.js LTS, Python, Go, Rust, Bun${NC}"
    $confirm = Read-Host "  Proceed? (y/n)"
    if ($confirm -ne "y" -and $confirm -ne "Y") {
        Write-Host "${DIM}  Cancelled.${NC}"
        return
...
function Uninstall-DevTools {
    ...
    Write-Host ""
    Write-Host "  Enter tool to uninstall: node, python, go, rust, bun"
