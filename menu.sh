...
#
# Usage:
#   Make executable: chmod +x single-embedded-checklist.sh
#   Run demo: ./single-embedded-checklist.sh --demo
#   Or call checklist from another script by sourcing this file.
#
# Output:
...
truncate_label() {
  label=$1; width=$2
  clean=$(printf '%s' "$label" | sed "s/$(printf '\033')\
...
# If script executed without args, show a small interactive example and then run a simulated install flow.
if [ -t 0 ]; then
  ...
  if selected=$(checklist "Install components" "Select components to install:" \
    ...
    # iterate over selected tags (one per line)
  ...
  # Non-interactive: print usage
  cat <<'USAGE'
Usage:
  ./single-embedded-checklist.sh --demo
    Run interactive demo.

  ./single-embedded-checklist.sh "Title" "Prompt" "tag|Label|on" "tag2|Label2|off" ...
    Run checklist directly with provided items.

  Source this file from another script to call checklist() function directly.

Notes:
  - Works in sh (dash/ash/busybox), bash, zsh.
  - For fish, run with --fish or let the dispatcher detect fish.
USAGE
  exit 0
fi
